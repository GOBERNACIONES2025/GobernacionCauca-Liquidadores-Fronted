...
@Component({
  ...
  templateUrl: './causales-anulacion.html',
  styleUrl: './causales-anulacion.css'
})
export class CausalesAnulacionComponent implements OnInit {
  private fb = inject(FormBuilder);
  public facade = inject(CausalesAnulacionFacade);
  public apiService = inject(CausalesAnulacionApiService);
  private toast = inject(ToastService);

  breadcrumbs = ['Configuración', 'Liquidación', 'Causales de Anulación'];
  ...
  causalesFiltradas = computed(() => this.facade.causalesAnulacion());

  counts = computed(() => {
    return {
      total: this.facade.totalCausalesAnulacion()
    };
  });
  ...
  cargarItems() {
    ...
    this.facade.cargarCausalesAnulacion(this.pageNumber(), this.pageSize(), this.searchText(), activo);
  }
