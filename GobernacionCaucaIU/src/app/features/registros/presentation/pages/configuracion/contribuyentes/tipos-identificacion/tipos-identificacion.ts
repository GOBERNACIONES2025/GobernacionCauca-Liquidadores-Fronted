...
export class TiposIdentificacion implements OnInit {
  ...
  public apiService = inject(TiposIdentificacionApiService);
  ...
  itemToToggle = signal<TipoIdentificacion | null>(null);
  ...
  // Filtered list
  tiposIdentificacionFiltrados = computed(() => this.facade.tiposIdentificacion());

  // Dynamic counts
  counts = computed(() => {
    return {
      total: this.facade.totalTiposIdentificacion()
    };
  });
  ...
  cargarItems() {
    ...
    this.facade.cargarTiposIdentificacion(this.pageNumber(), this.pageSize(), this.searchText(), activo);
  }
  ...
  edit(item: TipoIdentificacion) {
    ...
    this.apiService.obtenerPorId(item.id).subscribe({
      ...
      error: (err) => {
        this.loadingEditId.set(null);
        this.toast.error(formatUserErrorMessage(err, 'obtener el tipo de documento'));
        console.error(err);
      }
    });
  }

  promptToggleActivo(item: TipoIdentificacion) {
  ...
  executeToggleActivo() {
    ...
    this.facade.actualizar(item.id, {
      ...
      error: (err: any) => {
        this.isTogglingStatus.set(false);
        this.toast.error(formatUserErrorMessage(err, 'actualizar estado del tipo de documento'));
        console.error(err);
      }
    });
  }
    ...
    if (this.tipoIdentificacionForm.valid) {
      ...
      if (this.isEditMode) {
        this.facade.actualizar(this.selectedId!, {
          ...
          error: (err: any) => {
            this.toast.error(formatUserErrorMessage(err, 'actualizar el tipo de documento'));
            console.error(err);
          }
        });
      } else {
        this.facade.crear({
          ...
          error: (err: any) => {
            this.toast.error(formatUserErrorMessage(err, 'crear el tipo de documento'));
            console.error(err);
          }
        });
      }
