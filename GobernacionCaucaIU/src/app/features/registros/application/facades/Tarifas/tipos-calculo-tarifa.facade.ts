import { Injectable, inject, signal } from '@angular/core';
import { HttpErrorResponse } from '@angular/common/http';
import { Observable } from 'rxjs';
import { tap } from 'rxjs/operators';
import { TipoCalculoTarifa, CrearTipoCalculoTarifaRequest, ActualizarTipoCalculoTarifaRequest } from '../../../domain/models/Tarifas/tipo-calculo-tarifa.model';
import { TiposCalculoTarifaApiService } from '../../../infrastructure/api/Tarifas/tipos-calculo-tarifa-api.service';
import { ApiResponse } from '../../../../../core/shared/models/shared.model';

/**
 * @description
 * Facade (Capa de Aplicación) para la gestión del estado de Tipos de Cálculo de Tarifa.
 * Expone señales reactivas (Angular Signals) y métodos de orquestación de operaciones para la UI.
 */
@Injectable({
  providedIn: 'root'
})
export class TiposCalculoTarifaFacade {
  private apiService = inject(TiposCalculoTarifaApiService);

  // Signals
  readonly tiposCalculoTarifa = signal<TipoCalculoTarifa[]>([]);
  readonly totalTiposCalculoTarifa = signal<number>(0);
  
  // UI State
  readonly loading = signal<boolean>(false);
  readonly actionLoading = signal<boolean>(false);
  readonly error = signal<string | null>(null);
  readonly selectedTipoCalculoTarifa = signal<TipoCalculoTarifa | null>(null);

  /**
   * Carga la lista paginada de tipos de cálculo de tarifa.
   */
  cargarTiposCalculoTarifa(pageNumber: number = 1, pageSize: number = 10, search?: string, activo?: boolean): void {
    this.loading.set(true);
    this.error.set(null);

    this.apiService.obtenerTodos(pageNumber, pageSize, search, activo).subscribe({
      next: (response) => {
        if (response.success && response.data) {
          this.tiposCalculoTarifa.set(response.data.items || []);
          this.totalTiposCalculoTarifa.set(response.data.totalCount);
        } else {
          this.error.set(response.message || 'Error al cargar tipos de cálculo de tarifa');
        }
        this.loading.set(false);
      },
      error: (err: HttpErrorResponse) => {
        this.error.set(err.message || 'Error de conexión');
        this.loading.set(false);
      }
    });
  }

  /**
   * Selecciona un tipo de cálculo de tarifa por su ID.
   */
  seleccionarPorId(id: number): void {
    this.loading.set(true);
    this.error.set(null);
    this.selectedTipoCalculoTarifa.set(null);

    this.apiService.obtenerPorId(id).subscribe({
      next: (response) => {
        if (response.success && response.data) {
          this.selectedTipoCalculoTarifa.set(response.data);
        } else {
          this.error.set(response.message || 'Error al cargar tipo de cálculo de tarifa');
        }
        this.loading.set(false);
      },
      error: (err: HttpErrorResponse) => {
        this.error.set(err.message || 'Error de conexión');
        this.loading.set(false);
      }
    });
  }

  /**
   * Limpia la selección actual.
   */
  limpiarSeleccion(): void {
    this.selectedTipoCalculoTarifa.set(null);
  }

  /**
   * Crea un nuevo tipo de cálculo de tarifa.
   */
  crear(dto: CrearTipoCalculoTarifaRequest): Observable<ApiResponse<number>> {
    this.actionLoading.set(true);
    return this.apiService.crear(dto).pipe(
      tap({
        next: () => this.actionLoading.set(false),
        error: () => this.actionLoading.set(false)
      })
    );
  }

  /**
   * Actualiza un tipo de cálculo de tarifa existente.
   */
  actualizar(id: number, dto: ActualizarTipoCalculoTarifaRequest): Observable<void> {
    this.actionLoading.set(true);
    return this.apiService.actualizar(id, dto).pipe(
      tap({
        next: () => this.actionLoading.set(false),
        error: () => this.actionLoading.set(false)
      })
    );
  }

  /**
   * Elimina un tipo de cálculo de tarifa.
   */
  eliminar(id: number): Observable<void> {
    this.actionLoading.set(true);
    return this.apiService.eliminar(id).pipe(
      tap({
        next: () => {
          this.actionLoading.set(false);
          this.cargarTiposCalculoTarifa();
        },
        error: () => this.actionLoading.set(false)
      })
    );
  }
}
