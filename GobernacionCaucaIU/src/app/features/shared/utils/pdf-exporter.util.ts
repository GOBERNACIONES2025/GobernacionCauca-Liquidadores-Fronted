...
function createIsolatedIframe(htmlContent: string): Promise<{ iframe: HTMLIFrameElement; elementToPrint: HTMLElement; cleanup: () => void }> {
  return new Promise((resolve) => {
    ...
    // Permitir tiempo para el renderizado interno de tablas, imágenes y SVGs.
    // html2pdf clona el elemento en el documento principal; por eso copiamos dentro
    // del nodo imprimible los estilos declarados en <head>, evitando perder el diseño
    // de plantillas HTML autocontenidas al hacer ese clon.
    setTimeout(async () => {
      const targetElement = (doc.getElementById('pageContainer') || doc.body) as HTMLElement;
      const styles = Array.from(doc.querySelectorAll('style'));
      styles.reverse().forEach(style => targetElement.prepend(style.cloneNode(true)));
...
export async function generatePdfBlobFromHtml(htmlContent: string, options: PdfBlobOptions = {}): Promise<Blob> {
  ...
  try {
    const opt = {
      ...
      jsPDF: { unit: 'mm' as const, format: options.format ?? 'a4', orientation: 'portrait' as const }
    };
